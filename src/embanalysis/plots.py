...
def alt_title(data, **kwargs) -> str:
    return alt.TitleParams(
        text=data.meta.model_id,
        subtitle=data.meta.label,
        fontSize=24,
        subtitleFontSize=16,
        anchor="middle",
        **kwargs,
...
def plot_digit_overview(data, x_component=0, y_component=1) -> alt.Chart:
    ...
    return alt.vconcat(
        alt.hconcat(
            data.plot_by_digit(0, x_component, y_component),
            data.plot_by_digit(1, x_component, y_component),
        ).resolve_scale(color="independent"),
        alt.hconcat(
            data.plot_by_digit(2, x_component, y_component),
            data.plot_by_digit_length(x_component, y_component),
        ).resolve_scale(color="independent"),
...
def plot_component_patterns(
    ...
    component_dfs = []
    for i in range(min(n_components, len(data.pca.components_))):
        df = pd.DataFrame(
            {
                "Index": np.arange(min(n_values, len(data.pca.components_[i]))),
                "Value": data.pca.components_[
                    i, : min(n_values, len(data.pca.components_[i]))
                ],
...
def plot_top_correlated_components(data, n_vectors: int = 10) -> alt.Chart:
    ...
    """
    corr_df = data.top_correlations_df(n_vectors)
    charts = []
    for _, row in corr_df.iterrows():
        i, j = int(row["Component1"]), int(row["Component2"])
        chart = data.plot(x_component=i, y_component=j).properties(
            title=f"Components {i + 1} vs {j + 1} (corr={row['Correlation']:.2f})"
        )
        charts.append(chart)
        chart = data.plot_by_digit_length(x_component=i, y_component=j).properties(
...
def plot_correlation_heatmap(data, n_vectors: int = 20) -> alt.Chart:
    ...
    n_vectors = min(n_vectors, data.transformed.shape[1])
    correlations = np.corrcoef(data.transformed[:, :n_vectors].T)
...
def plot_explained_variance(data) -> alt.Chart:
    ...
    return (
        alt.Chart(data.data.variance_df)
